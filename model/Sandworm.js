...
class Sandworm {
  ...
  move() {
    ...
    if (Math.abs(target.x - head.x) > 10) {
        newHead.x = head.x + (target.x - head.x) / Math.abs(target.x - head.x) * velocity;
    }
    else if (Math.abs(target.y - head.y) > 10) {
        newHead.y = head.y + (target.y - head.y) / Math.abs(target.y - head.y) * velocity;
    }
  ...
  checkCollision() {
    const target = this.target;
    const distanceSquare = (this.body[0].x-target.x)**2 + (this.body[0].y-target.y)**2;
    if(distanceSquare < 1000){
        console.log(distanceSquare);
